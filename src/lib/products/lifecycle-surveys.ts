...
export const LIFECYCLE_PHASES: LifecyclePhaseDef[] = [
  {
    ...
    window_days: 3,
    intent:
      "Capture install friction, first-run clarity, missing docs — highest leverage for onboarding.",
    questions: [
      scale(
        "setup_ease",
        "How easy was setup / install (export → load into your agent)?",
        "install",
        "Onboarding friction",
      ),
      scale(
        "first_tick_clarity",
        "How clear was the first tick / first use of Kernel or Loop?",
        "general",
        "Time-to-value",
      ),
      single(
      ...
      text(
        "first_hour_fail",
        "What failed or felt wrong in the first hour?",
        "general",
      ...
      ),
      {
        id: "wtp_alive_usd",
        prompt:
          "Honest max USD you would pay for what you just installed (Alive or SKU). $0 is a valid answer.",
        type: "currency",
        required: true,
        min: 0,
        max: 5000,
      ...
      single(
        ...
        "At $14.99 / $19.99 / $29.99 founding, would you buy when payments open?",
        ["yes", "no", "maybe"],
        "commerce",
        "Founding demand",
      ),
  ...
  },
  {
    ...
    questions: [
      ...
      text(
        "would_change_now",
        "If we could change one thing for YOUR agent this week, what?",
        "general",
