    ...
    files.push({
      ...
      content: skillMd(
        ...
        [
          ...
          "",
          "## Clarity (read this first)",
          "1. Kernel is already loaded (system_prompt_short).",
          "2. One **tick** = run phases in order (Producer work → Critic score).",
          "3. **Promote gate**: only accept work when Critic ≥ 0.7 — else replan.",
          "4. After promote: deposit_outcome / leave_trace so Dual ranking improves.",
          "5. HTTP: POST /api/products/run { token, action: \"tick\" } for one platform-assisted tick.",
          "",
          "## When to use",
          "- Every major work cycle after kernel load — not free-form chat",
          "",
          "## Progressive load",
          "- `instructions.md` — full agent instructions (start here)",
