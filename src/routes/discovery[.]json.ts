      ...
      GET: async ({ request }) => {
        ...
        const body = {
          ...
          surfaces: {
            ...
            agent_descriptions: `${origin}/.well-known/agent-descriptions`,
            a2a_rpc: `${origin}/api/a2a`,
            ard_search: `${origin}/api/ard/search`,
            activity_feed: `${origin}/api/feed`,
            mcp_server_card: `${origin}/.well-known/mcp/server-card.json`,
            mcp_server_card_alt: `${origin}/.well-known/mcp/server-card`,
            mcp_registry_package: `${origin}/api/mcp-registry/server.json`,
            mcp_publish_status: `${origin}/api/mcp-registry/publish-status`,
            dns_mcp_status: `${origin}/api/dns/mcp-status`,
            agentmap: `${origin}/agentmap.json`,
          ...
          federation: {
            ...
            model: "ingest-augment-score + inbound self-list + ARD bidirectional",
            consume: `${origin}/api/catalog`,
            ard_catalog: `${origin}/.well-known/ai-catalog.json`,
            ard_search: `${origin}/api/ard/search?q={task}&federation=auto`,
            ...
            how_agents_find_us: [
              ...
              `${origin}/.well-known/signature-agent-card.json`,
              `${origin}/api/products/agentfinder`,
            ],
          },
